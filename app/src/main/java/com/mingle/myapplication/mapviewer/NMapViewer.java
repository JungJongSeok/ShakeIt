...
public class NMapViewer extends NMapActivity {
	...
	private static final String APP_KEY = "4OUr6eIicnv3_ebCax81";
	private static final String CLIENT_ID = "FqrI3qdVDv";
	...
	private void testFloatingPOIdataOverlay() {
		...
		int valueNum = intent.getIntExtra(ITEM_VALUE, 0);
		String str = "";
		if(valueNum == 1){
			str = "모자";
		}
		...
		for(int i=1;i<6;i++) {
			poiData.addPOIitem(127 + Math.random()/10, 37 + Math.random()/10, "" + i + "번째 "+str+"가게", marker1, 0);
		}
		poiData.endPOIdata();
